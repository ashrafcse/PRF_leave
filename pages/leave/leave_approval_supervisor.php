...
if ($employeeId === 0) {
    // Use JavaScript redirect since we might not have header.php included yet
...
// First include header.php to get database connection
...
function getApprovalFormData($conn, $supervisorID, $leaveApplicationID, $loggedInEmployeeID) {
    ...
    if ($isAuthorizedSupervisor && $isAuthorizedForThisLeave && $currentApprovalStatus !== 'rejected') {
        switch ($supervisorType) {
            case 'admin':
                // Admin supervisor can approve as L1 - even if L2 already approved
                $canApproveL1 = true;
                $canReject = true;
                break;
            ...
            case 'technical':
            case 'second_level':
                // Technical or 2nd level supervisor can approve as L2 - even if L1 already approved
                $canApproveL2 = true;
                $canReject = true;
                break;
        }
...
// ==============================================
// Function to check if leave is pending for current supervisor
// ==============================================
function isLeavePendingForSupervisor($conn, $leaveApplicationID, $supervisorID, $supervisorType) {
    try {
        // Get approval status
        $sql = "
            SELECT 
        ...
        error_log("Error checking pending status: " . $e->getMessage());
        return false;
    }
}

// ==============================================
// Function to get supervisor type for a specific employee
// ==============================================
function getSupervisorTypeForEmployee($conn, $supervisorID, $employeeID) {
    try {
        // Check if supervisor is admin supervisor for this employee
        $sqlAdmin = "SELECT COUNT(*) as cnt FROM [dbPRFAssetMgt].[dbo].[Employees] WHERE EmployeeID = :employee_id AND SupervisorID_admin = :supervisor_id";
        $stmtAdmin = $conn->prepare($sqlAdmin);
        $stmtAdmin->bindParam(':employee_id', $employeeID, PDO::PARAM_INT);
        $stmtAdmin->bindParam(':supervisor_id', $supervisorID, PDO::PARAM_INT);
        $stmtAdmin->execute();
        $resultAdmin = $stmtAdmin->fetch(PDO::FETCH_ASSOC);
        if ($resultAdmin['cnt'] > 0) {
            return 'admin';
        ...
        // Check if supervisor is technical supervisor for this employee
        $sqlTech = "SELECT COUNT(*) as cnt FROM [dbPRFAssetMgt].[dbo].[Employees] WHERE EmployeeID = :employee_id AND SupervisorID_technical = :supervisor_id";
        $stmtTech = $conn->prepare($sqlTech);
        $stmtTech->bindParam(':employee_id', $employeeID, PDO::PARAM_INT);
        $stmtTech->bindParam(':supervisor_id', $supervisorID, PDO::PARAM_INT);
        $stmtTech->execute();
        $resultTech = $stmtTech->fetch(PDO::FETCH_ASSOC);
        if ($resultTech['cnt'] > 0) {
            return 'technical';
        ...
        // Check if supervisor is 2nd level supervisor for this employee
        $sqlSecond = "SELECT COUNT(*) as cnt FROM [dbPRFAssetMgt].[dbo].[Employees] WHERE EmployeeID = :employee_id AND SupervisorID_2ndLevel = :supervisor_id";
        $stmtSecond = $conn->prepare($sqlSecond);
        $stmtSecond->bindParam(':employee_id', $employeeID, PDO::PARAM_INT);
        $stmtSecond->bindParam(':supervisor_id', $supervisorID, PDO::PARAM_INT);
        $stmtSecond->execute();
        $resultSecond = $stmtSecond->fetch(PDO::FETCH_ASSOC);
        if ($resultSecond['cnt'] > 0) {
            return 'second_level';
        ...
        error_log("Error getting supervisor type: " . $e->getMessage());
        return null;
    }
}
...
function showPendingLeavesList($conn, $supervisorID, $loggedInEmployeeID, $username, $loggedInEmployee, $showApprovalForm = false, $approvalData = null) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $showApprovalForm && $approvalData) {
        ...
        if (empty($comments)) {
            ...
            try {
                ...
                // Store success message in session and redirect to main page
                $_SESSION['success_msg'] = $successMsg;
                
                // Use JavaScript redirect since we already have output from header.php
                echo "<script>window.location.href = 'leave_approval_supervisor.php';</script>";
                exit;
    ...
    // Fetch ALL leaves for DIRECT REPORTS (we'll filter pending in PHP)
    try {
        // Get ALL leaves from direct reports (Status 0, 1, 2, 3)
        $sqlAllLeaves = "
            SELECT 
        ...
        $stmtAllLeaves = $conn->prepare($sqlAllLeaves);
        $stmtAllLeaves->execute([$supervisorID, $supervisorID, $supervisorID]);
        $allLeaves = $stmtAllLeaves->fetchAll(PDO::FETCH_ASSOC);
        
    } catch (PDOException $e) {
        // If query fails, show empty list
        $allLeaves = [];
        error_log("Error fetching all leaves: " . $e->getMessage());
    }
    
    // Separate leaves into pending and approved based on current supervisor's perspective
    $pendingLeaves = [];
    $approvedLeaves = [];
    ...
    foreach ($allLeaves as $leave) {
        ...
        // Get supervisor type for this specific employee
        $supervisorType = getSupervisorTypeForEmployee($conn, $supervisorID, $employeeID);
        
        if ($supervisorType) {
            // Check if this leave is still pending for this supervisor
            $isPending = isLeavePendingForSupervisor($conn, $leaveID, $supervisorID, $supervisorType);
    ...
    <head>
        ...
        <script>
            ...
            document.addEventListener('DOMContentLoaded', function() {
                ...
                // If there's a success message, scroll to top to show it
                <?php if ($successMsg): ?>
                ...
                <div class="main-content">
                    ...
                    <div class="section">
                        ...
                        <!-- <div class="alert-info">
                            ...
                            Even if one supervisor has already approved, the other can still approve.
                        </div> -->
                        ...
                        <?php else: ?>
                           <div class="alert alert-success">
                                <p><strong>No pending leave applications found</strong></p>
                                <!-- <p>All leave applications from your direct reports have been processed.</p> -->
                            </div> 
                        <?php endif; ?>
                    </div>
                    ...
                    <div class="section">
                        ...
                        <div id="approvedLeavesTable" style="display: none;">
                            <?php if (count($approvedLeaves) > 0): ?>
                                <div class="alert-info">
                                    <strong>Note:</strong> These are leave applications that you have already processed or that have been rejected.
                                </div>
...
// ==============================================
// MAIN EXECUTION - Show the content
// ==============================================

...
$supervisorEmployeeCodes = [
    ...
    '0018',  // Example supervisor 4
    '0107', 
    '0358', 
    '0505', 
    '0130', 
      // Example supervisor 5
...
// ==============================================
// Check if logged-in user is a supervisor dynamically
// ==============================================

// 1. Get logged-in employee ID and EmployeeCode
$sqlEmployee = "
...
// 2. Check if any employee has this employee as their supervisor
$sqlSupervisorCheck = "
    SELECT COUNT(*) as cnt
    FROM [dbPRFAssetMgt].[dbo].[Employees]
    WHERE SupervisorID_admin = :emp_admin
       OR SupervisorID_technical = :emp_tech
       OR SupervisorID_2ndLevel = :emp_2nd
";
$stmtCheck = $conn->prepare($sqlSupervisorCheck);
$stmtCheck->bindParam(':emp_admin', $loggedInEmployeeID, PDO::PARAM_INT);
$stmtCheck->bindParam(':emp_tech', $loggedInEmployeeID, PDO::PARAM_INT);
$stmtCheck->bindParam(':emp_2nd', $loggedInEmployeeID, PDO::PARAM_INT);
$stmtCheck->execute();
$result = $stmtCheck->fetch(PDO::FETCH_ASSOC);

$isSupervisor = ($result['cnt'] > 0);
...
function showApprovalPage($conn, $supervisorID, $leaveApplicationID, $loggedInEmployeeID, $username, $loggedInEmployee, $manualMode = false, $manualCode = '', $supervisorList = []) {
    ...
    if ($isL1Supervisor && $leave['SupervisorID_admin'] == $supervisorID) {
        $isAuthorizedForThisLeave = true;
    } elseif ($isL2Supervisor && $leave['SupervisorID_technical'] == $supervisorID) {
        $isAuthorizedForThisLeave = true;
    } elseif ($is2ndLevelSupervisor && $leave['SupervisorID_2ndLevel'] == $supervisorID) {
        $isAuthorizedForThisLeave = true;
    }

    // Check current approval status - fetch all three variables
    $sqlApproval = "
        SELECT 
            ApprovalID,
            L1ApprovedBy,
    ...
    // Get the three variables from the database
    $leaveReason = isset($leave['Reason']) ? $leave['Reason'] : '';

// 2. RejectionReason from LeaveApprovals table
$rejectionReason = isset($approval['RejectionReason']) ? $approval['RejectionReason'] : '';

// 3. Comments (L1 Supervisor Comments) from LeaveApprovals table
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (empty($comments)) {
            ...
            try {
                ...
                // Update the three variables after refresh
...
$l1SupervisorComments = isset($approval['Comments']) ? $approval['Comments'] : '';
        ...
        <div class="container">
            ...
            <div class="content">
                <div class="main-content">
                    ...
                    <div class="alert alert-warning">
                        <strong>Warning:</strong> You are authorized as a supervisor, but this leave application is not from an employee assigned to you.
                    ...
                    <div class="section">
                        ...
                        <!-- 1. Reason from LeaveApplications table -->
                        <div class="info-item">
                            <span class="info-label">Leave Reason (Applicant):</span>
                            <span class="info-value">
                                <div class="data-box">
                                    <?php echo !empty($leaveReason) ? nl2br(htmlspecialchars($leaveReason)) : '<span style="color: #6c757d; font-style: italic;">No reason provided</span>'; ?>
                                </div>
                            </span>
                        </div>
                    </div>
                    ...
                    <div class="section">
                        ...
                        <div class="info-grid">
                            <div class="info-item">
                                ...
                                <span class="info-value">
    <?php 
        echo htmlspecialchars(isset($leave['SupervisorID_admin']) ? $leave['SupervisorID_admin'] : 'Not assigned'); 
    ?>
    <?php if (isset($leave['SupervisorID_admin']) && $leave['SupervisorID_admin'] == $supervisorID && $isL1Supervisor): ?>
    ...
    <span class="info-value">
        <?php 
            echo htmlspecialchars(isset($leave['SupervisorID_technical']) ? $leave['SupervisorID_technical'] : 'Not assigned'); 
        ?>
        <?php if (isset($leave['SupervisorID_technical']) && $leave['SupervisorID_technical'] == $supervisorID && $isL2Supervisor): ?>
            <span class="badge badge-authorized">Your Assignment</span>
        <?php endif; ?>
    </span>
</div>
...
<div class="info-item">
    ...
    <span class="info-value">
        <?php 
            echo htmlspecialchars(isset($leave['SupervisorID_2ndLevel']) ? $leave['SupervisorID_2ndLevel'] : 'Not assigned'); 
        ?>
        <?php if (isset($leave['SupervisorID_2ndLevel']) && $leave['SupervisorID_2ndLevel'] == $supervisorID && $is2ndLevelSupervisor): ?>
            <span class="badge badge-authorized">Your Assignment</span>
        <?php endif; ?>
    </span>
</div>
                    ...
                    <!-- Approval Status with 3 new variables -->
                    <div class="section">
                        ...
                        <!-- 3. L1 Supervisor Comments from LeaveApprovals table -->
                        <?php if (!empty($l1SupervisorComments)): ?>
                        <div class="info-item">
                            <span class="info-label">L1 Supervisor Comments:</span>
                            <span class="info-value">
                                <div class="data-box">
                                    <?php echo nl2br(htmlspecialchars($l1SupervisorComments)); ?>
                                </div>
                            </span>
                        </div>
                        <?php endif; ?>
                    ...
                    <div class="section">
                        ...
                        <form method="post" action="">
                            <div class="form-group">
                                ...
                                <textarea id="comments" name="comments" rows="3" required placeholder="Enter your comments about this leave application..."><?php echo htmlspecialchars($l1SupervisorComments); ?></textarea>
                            </div>
                            
                            <div class="form-group">
                                <label for="rejection_reason">Rejection Reason (required if rejecting):</label>
                                <textarea id="rejection_reason" name="rejection_reason" rows="2" placeholder="Please provide reason for rejection if applicable..."><?php echo htmlspecialchars($rejectionReason); ?></textarea>
                            </div>
                    ...
                    <div class="section">
                        ...
                        <p style="text-align: center; color: #666; padding: 20px;">
                            ...
                            <?php if (!$isAuthorizedSupervisor): ?>
                                You are not authorized to approve/reject leaves. Only authorized supervisors can take actions.
                            <?php else: ?>
...
function showPendingLeavesList($conn, $supervisorID, $loggedInEmployeeID, $username, $loggedInEmployee, $manualMode = false, $manualCode = '', $supervisorList = []) {
    ...
    <?php if (isset($supervisorInfo['EmployeeCode']) && in_array($supervisorInfo['EmployeeCode'], $supervisorList)): ?>
    ...
    <div class="section">
        <h2>
            Pending Leave Applications from Your Supervisee
            (<?php echo count($pendingLeaves); ?>)
        </h2>

        <?php if (isset($loggedInEmployee['EmployeeCode']) && in_array($loggedInEmployee['EmployeeCode'], $supervisorList)): ?>
            <div class="alert-info">
                <strong>Supervisor View:</strong>
                You are viewing leave applications only from employees assigned to you.
            </div>
        <?php endif; ?>
                    ...
                    <?php foreach ($pendingLeaves as $leave): ?>
                        <tr>
                            ...
                            <td>
                                ...
                                $url = '?leave_id=' . $leaveId . '&supervisor_id=' . (int)$supervisorID;

                                if ($manualMode && !empty($manualCode)) {
                                    $url .= '&manual_code=' . urlencode(trim($manualCode));
                                }
                                ?>
                                <a href="<?php echo $url; ?>" class="btn-action">
                                    Approve / Reject
                                </a>
                            </td>
                        </tr>
    ...
    <div class="info-item">
        ...
        <span class="info-value">
            <?php echo htmlspecialchars(isset($loggedInEmployee['FullName']) ? $loggedInEmployee['FullName'] : ''); ?>
        </span>
    </div>
    <div class="info-item">
        <span class="info-label">Employee Code:</span>
        <span class="info-value">
            <?php echo htmlspecialchars(isset($loggedInEmployee['EmployeeCode']) ? $loggedInEmployee['EmployeeCode'] : ''); ?>
            <?php if (isset($loggedInEmployee['EmployeeCode']) && in_array($loggedInEmployee['EmployeeCode'], $supervisorList)): ?>

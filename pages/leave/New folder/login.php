...
// jodi already logged-in hoy, direct redirect
if (current_user()) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  // try_login already tomar system e ase
  [$ok, $msg] = try_login($conn, $username, $password);

  if ($ok) {

    // ===== RBAC er jonno user_id session e set kora =====
    if (session_status() === PHP_SESSION_NONE) {
        session_start();
    }

    // try_login success howar pore current_user() theke info niye ashi
    $u  = current_user();
    $id = null;

    if (is_array($u)) {
        if (isset($u['UserID'])) {
            $id = $u['UserID'];       // jodi column nam UserID hoy
        } elseif (isset($u['user_id'])) {
            $id = $u['user_id'];      // jodi user_id hoy
        } elseif (isset($u['id'])) {
            $id = $u['id'];           // jodi sudhu id hoy
        }
    }

    if ($id !== null) {
        // helpers.php er current_user_id() etai use korbe
        $_SESSION['user_id'] = (int)$id;
        // permission cache clear (notun login e notun kore load korte)
...
<style>
  :root{
    ...
    --accent2:#06b6d4; /* cyan   */
    --field:#0b1220;   /* input bg (translucent) */
  ...
  *{box-sizing:border-box}
  ...
  /* animated transform background blob */
  .blob{
  ...
  @keyframes drift{
    ...
    to{   transform: translate(-5%,-8%) rotate(20deg) scale(1.15);}
  }

  /* glass card (no white background) */
  .card{
    width:100%; max-width:420px;  /* narrower desktop width */
  ...
  .alert{
    background:rgba(248,113,113,.15); color:#fecaca;
  ...
  .input:focus{
    border-color:var(--ring); box-shadow:0 0 0 4px rgba(99,102,241,.25);
  ...
  .toggle{
    ...
    border:1px solid rgba(148,163,184,.25); border-radius:10px; color:#e2e8f0;
    cursor:pointer; font-size:16px; backdrop-filter: blur(3px);
  }

  .btn{
    width:100%; padding:12px 14px; border:none; border-radius:14px;
    cursor:pointer; color:#0b1220; font-weight:800; letter-spacing:.2px; font-size:15px;
  ...
  .row{
    ...
    gap:8px; margin-top:12px; color:var(--muted); font-size:12px;
  }
        ...
        <div class="group">
          ...
          <div class="input-wrap">
            ...
            <input class="input" id="username" name="username" type="text"
                   required autocomplete="username" placeholder="your.username" autofocus>
          </div>
        </div>
        ...
        <div class="group">
          ...
          <div class="input-wrap">
            ...
            <button type="button" class="toggle"
                    aria-controls="password" aria-label="Show password" title="Show password"
...
<script>
  function togglePw(btn){
    const input = document.getElementById(btn.getAttribute('aria-controls') || 'password');
    if(!input) return;

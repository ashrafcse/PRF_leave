<?php
/***********************
 * Departments - Full CRUD (same design + auto search/filter)
 ***********************/
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['act']) && $_POST['act'] === 'create') {
  check_csrf();
  $name = isset($_POST['DepartmentName']) ? trim($_POST['DepartmentName']) : '';
  ...
  if ($name !== '') {
    try {
      ...
      $stmt = $conn->prepare("
        INSERT INTO dbo.Departments (DepartmentName, CreatedAt, CreatedBy, IsActive)
        VALUES (:n, GETDATE(), :by, :a)
      ");
      ...
      $msg_type = 'danger';
      $msg = ($e->getCode() == '23000')
        ? "Create failed: Duplicate department name."
...
if ($edit_id > 0) {
  try {
    $st = $conn->prepare("
      SELECT d.DepartmentID, d.DepartmentName, d.CreatedAt, d.CreatedBy, d.IsActive,
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['act']) && $_POST['act'] === 'update') {
  check_csrf();
  $id   = isset($_POST['DepartmentID']) ? (int)$_POST['DepartmentID'] : 0;
  $name = isset($_POST['DepartmentName']) ? trim($_POST['DepartmentName']) : '';
  $name = trim(preg_replace('/\s+/', ' ', $name));
  $isActive = isset($_POST['IsActive']) ? 1 : 0;
  ...
  if ($id > 0 && $name !== '') {
    try {
      ...
      ");
      $stmt->execute(array(':n'=>$name, ':a'=>$isActive, ':id'=>$id));
      header('Location: ' . $self); exit;
    } catch (PDOException $e) {
      $msg_type = 'danger';
      $msg = ($e->getCode() == '23000')
        ? "Update failed: Duplicate department name."
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['act']) && $_POST['act'] === 'toggle') {
  ...
  if ($id > 0) {
    try {
      $stmt = $conn->prepare("UPDATE dbo.Departments SET IsActive = :a WHERE DepartmentID = :id");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['act']) && $_POST['act'] === 'delete') {
  ...
  if ($id > 0) {
    try {
      $stmt = $conn->prepare("DELETE FROM dbo.Departments WHERE DepartmentID = :id");
...
/* 5) Query list (always full list; auto filter via JS) */
try {
  $rows = $conn->query("
    SELECT d.DepartmentID, d.DepartmentName, d.CreatedAt, d.CreatedBy, d.IsActive,
           u.Username AS CreatedByUsername
  ...
  <div class="d-flex flex-wrap align-items-center justify-content-between mb-3 gap-2">
    <div>
      ...
      <div class="page-subtitle">
        PRF department তথ্য manage করুন – create, update, status toggle & delete.
      </div>
    </div>
    <span class="badge-soft">
      <i class="fas fa-layer-group"></i>
      Total Departments: <?php echo count($rows); ?>
    </span>
  </div>
    ...
    <div class="card-body">
      <?php if (!empty($editRow)): ?>
        ...
        <form method="post" accept-charset="UTF-8">
          ...
          <input type="hidden" name="DepartmentID" value="<?php echo (int)$editRow['DepartmentID']; ?>">

          <div class="row g-3">
            <div class="col-12 col-md-8">
              <label class="form-label">Department Name</label>
              <input type="text" name="DepartmentName" class="form-control" required value="<?php echo h($editRow['DepartmentName']); ?>">
            </div>
            <div class="col-12 col-md-4 d-flex align-items-end">
              <div class="form-check">
                <input class="form-check-input" type="checkbox" name="IsActive" id="isActiveEdit" <?php echo ((int)$editRow['IsActive']===1?'checked':''); ?>>
      ...
      <?php else: ?>
        ...
        <form method="post" class="row g-3" accept-charset="UTF-8">
          ...
          <div class="col-12 col-md-8">
            <label class="form-label">Department Name</label>
            <input type="text" name="DepartmentName" class="form-control" required placeholder="e.g. HR / Accounts">
          </div>
          <div class="col-12 col-md-4 d-flex align-items-end">
            <div class="form-check">
  ...
  <div class="card card-elevated">
    <div class="card-body">
      ...
      <div class="table-responsive">
        <table class="table table-hover align-middle" id="deptTable">
          <thead>
            <tr>
              <th>#</th>
              <th>Name</th>
          ...
          <?php foreach($rows as $r): ?>
            ...
            <tr data-status="<?php echo $isActive; ?>"
              ...
              <td><?php echo (int)$r['DepartmentID']; ?></td>
              ...
              <td class="text-end">
                <div class="action-stack">
                  <a class="btn btn-muted btn-sm w-100 w-md-auto"
                     href="<?php echo h($self); ?>?edit=<?php echo (int)$r['DepartmentID']; ?>">
                  ...
                  <form method="post" class="d-inline"
                    ...
                    <input type="hidden" name="DepartmentID" value="<?php echo (int)$r['DepartmentID']; ?>">
                  ...
                  <form method="post" class="d-inline"
                        onsubmit="return confirm('Delete this department permanently?');"
                        accept-charset="UTF-8">
                    <input type="hidden" name="csrf" value="<?php echo h($CSRF); ?>">

 ...
 * - Executes leave balance stored procedure after creating employee
 ***********************/
...
function create_employee(PDO $conn, array $p) {
    ...
    try {
        ...
        // ======================= EXECUTE LEAVE BALANCE STORED PROCEDURE =======================
       // ======================= EXECUTE LEAVE BALANCE STORED PROCEDURE =======================
try {
    // Execute the stored procedure to generate leave balance for new employee
    $procSql = "EXEC [dbPRFAssetMgt].[dbo].[usp_GenLeaveBalanceForEmployee] @EmployeeID = :empId";
    $procStmt = $conn->prepare($procSql);
    $procStmt->execute([':empId' => $employeeId]);
    
    // Also add for next year if needed (optional)
    $nextYearProcSql = "
        INSERT INTO [dbPRFAssetMgt].[dbo].[LeaveBalances] ( 
            EmployeeID, 
            LeaveTypeID, 
            LeaveYear, 
            OpeningBalance, 
        ...
        SELECT 
            ...
            LT.MaxDaysPerYear,
            0.00
        FROM dbPRFAssetMgt.dbo.LeaveTypes AS LT
        WHERE LT.IsActive = 1
          AND NOT EXISTS (
              SELECT 1 
              FROM dbPRFAssetMgt.dbo.LeaveBalances LB 
              WHERE LB.EmployeeID = :empId 
                AND LB.LeaveTypeID = LT.LeaveTypeID 
                AND LB.LeaveYear = YEAR(GETDATE()) + 1
          )
    ";
    $nextYearStmt = $conn->prepare($nextYearProcSql);
    $nextYearStmt->execute([':empId' => $employeeId]);
    
    // Log success
    error_log("Leave balance stored procedure executed for EmployeeID: " . $employeeId);
} catch (Exception $procE) {
    // Log error but don't fail the main operation
    error_log("Failed to execute leave balance stored procedure for EmployeeID {$employeeId}: " . $procE->getMessage());
    // Try alternative method
    try {
        // Direct SQL as fallback
        $fallbackSql = "
            INSERT INTO [dbPRFAssetMgt].[dbo].[LeaveBalances] ( 
                EmployeeID, 
                LeaveTypeID, 
            ...
            WHERE LT.IsActive = 1
              AND NOT EXISTS (
                  ...
                  WHERE LB.EmployeeID = :empId 
                    AND LB.LeaveTypeID = LT.LeaveTypeID 
                    AND LB.LeaveYear = YEAR(GETDATE())
              )
        ";
        $fallbackStmt = $conn->prepare($fallbackSql);
        $fallbackStmt->execute([':empId' => $employeeId]);
        error_log("Used fallback SQL for leave balance for EmployeeID: " . $employeeId);
    } catch (Exception $fallbackE) {
        $msg .= " Note: Leave balance generation failed. Error: " . $fallbackE->getMessage();
    }
}
// ======================= END STORED PROCEDURE EXECUTION =======================
        // ======================= END STORED PROCEDURE EXECUTION =======================
...
if ($_SERVER['REQUEST_METHOD']==='POST' && (isset($_POST['act']) ? $_POST['act'] : '')==='create'){
    ...
    if (!empty($validationErrors)){
        ...
        try{
            ...
            $msg = "Employee created successfully. Leave balance has been automatically generated.";
...
<div class="page-wrap">
  <div class="d-flex flex-wrap align-items-center justify-content-between mb-3 gap-2">
    <div>
      ...
      <div class="validation-note"><i class="fas fa-calendar-check me-1"></i>Leave balance is automatically generated when creating new employees</div>
    </div>

 ...
 * Schema-aligned with dbo.Employees
 ***********************/
...
const COL_LIMITS = [
    'EmployeeCode'          => 50,   // nvarchar(50)
    'FirstName'             => 100,  // nvarchar(100)
    'LastName'              => 100,  // nvarchar(100)
    'NationalID'            => 100,  // nvarchar(100)
    'Email_Office'          => 200,  // nvarchar(200)
    'Email_Personal'        => 50,   // varchar(50)
    'Phone1'                => 15,   // varchar(15)
    'Phone2'                => 15,   // varchar(15)
    'Blood_group'           => 3,    // char(3)
    'Salary_increment_month'=> 10,   // varchar(10)
    'Job_Type'              => 50,   // varchar(50)
    'Salary_level'          => 10,   // varchar(10)
    'Status'                => 50    // nvarchar(50)
];
...
function supervisors_by_role(PDO $conn, $roleName) {
    $sql = "
        WITH RU AS (
...
function update_employee(PDO $conn, $id, array $p) {
    ...
    $stmt->execute([
        ':EmployeeCode'           => trim((string)$p['EmployeeCode']),
        ':FirstName'              => trim((string)$p['FirstName']),
        ':LastName'               => as_null(isset($p['LastName'])?$p['LastName']:''),   // NULL ok
        ':NationalID'             => as_null(isset($p['NationalID'])?$p['NationalID']:''), 
        ':Email_Office'           => as_null(isset($p['Email_Office'])?$p['Email_Office']:''), 
        ':Email_Personal'         => as_null(isset($p['Email_Personal'])?$p['Email_Personal']:''), 
        ':Phone1'                 => as_null(isset($p['Phone1'])?$p['Phone1']:''), 
...
if ($_SERVER['REQUEST_METHOD']==='POST' && (isset($_POST['act']) ? $_POST['act'] : '')==='update'){
    ...
    if ($id<=0){
        ...
        $missing=array();
        foreach(['EmployeeCode','FirstName','JobTitleID','DepartmentID','LocationID','Status'] as $k){
            $v = isset($payload[$k]) ? $payload[$k] : '';
            if($v==='' || (is_int($v) && $v===0)) $missing[]=$k;
        }
        if (!empty($missing)){
...
if ($_SERVER['REQUEST_METHOD']==='POST' && (isset($_POST['act']) ? $_POST['act'] : '')==='delete'){
    ...
    if($id>0){
        try{
            $conn->prepare("DELETE FROM dbo.Employees WHERE EmployeeID=:id")->execute([':id'=>$id]);
...
/* ---- List (Current list of employees) ---- */
try{
    $where  = array();
    $params = array();
    ...
    $sql .= " ORDER BY e.CreatedAt DESC, e.EmployeeID DESC";
    ...
    $rows=[];
    $msg="Load list failed: ".h(errfmt($e)); $msg_type='danger';
}
...
<div class="page-wrap">
  ...
  <?php if ($msg): ?>
    <div class="alert alert-<?php echo ($msg_type==='danger'?'danger':'success'); ?> alert-dismissible fade show shadow-sm" role="alert">
      <?php echo $msg; ?>
      <?php if ($notes): ?>
        <div class="mt-1 small text-muted"><?php echo h(implode(' ', $notes)); ?></div>
      <?php endif; ?>
    ...
    <div class="card-body">
      <?php if (!empty($editRow)): ?>
        ...
        <form method="post" accept-charset="UTF-8">
          ...
          <div class="row g-3">
            <div class="col-md-3">
              <label class="form-label">PRF ID (Employee Code) *</label>
            ...
            <div class="col-md-3">
              ...
              <select name="Blood_group" class="form-select">
                ...
                <?php foreach ($BLOOD_GROUPS as $bg): ?>
                  <option value="<?php echo h($bg); ?>" <?php echo ($editRow['Blood_group']===$bg?'selected':''); ?>>
                    <?php echo h($bg); ?>
                  </option>
                <?php endforeach; ?>
              </select>
            ...
            <div class="col-md-3">
              ...
              <select name="Salary_increment_month" class="form-select">
                ...
                <?php foreach ($INCREMENT_MONTHS as $code=>$label): ?>
                  <option value="<?php echo h($code); ?>" <?php echo ($editRow['Salary_increment_month']===$code?'selected':''); ?>>
                    <?php echo h($label); ?>
                  </option>
                <?php endforeach; ?>
              </select>
            ...
            <div class="col-md-3">
              ...
              <select name="Job_Type" class="form-select">
                ...
                <?php foreach ($JOB_TYPES as $jt): ?>
                  <option value="<?php echo h($jt); ?>" <?php echo ($editRow['Job_Type']===$jt?'selected':''); ?>>
                    <?php echo h($jt); ?>
                  </option>
                <?php endforeach; ?>
              </select>
            ...
            <div class="col-md-3">
              ...
              <select name="Salary_level" class="form-select">
                ...
                <?php foreach ($SALARY_LEVELS as $lvl): ?>
                  <option value="<?php echo h($lvl); ?>" <?php echo ($editRow['Salary_level']===$lvl?'selected':''); ?>>
                    <?php echo h($lvl); ?>
                  </option>
                <?php endforeach; ?>
              </select>
            ...
            <div class="col-md-3">
              ...
              <select name="Salary_Steps" class="form-select">
                <option value="">-- Select --</option>
                <?php
                  $currentStep = isset($editRow['Salary_Steps']) ? (int)$editRow['Salary_Steps'] : 0;
                  for ($i=1; $i<=15; $i++):
                    $sel = ($currentStep === $i) ? ' selected' : '';
                ?>
      ...
      <?php else: ?>
        ...
        <form method="post" class="row g-3" accept-charset="UTF-8">
          ...
          <div class="col-md-3">
            <label class="form-label">Phone 1</label>
            <input type="text" name="Phone1" class="form-control" maxlength="15">
          </div>
          <div class="col-md-3">
            <label class="form-label">Phone 2</label>
            <input type="text" name="Phone2" class="form-control" maxlength="15">
          </div>
    ...
    <div class="card-body">
      <div class="d-flex align-items-center justify-content-between flex-wrap gap-2 mb-2">
        ...
        <span class="filters-helper">
          ...
          Filters ব্যবহার করে দ্রুত narrow down করতে পারেন
        </span>
      </div>
              ...
              <tr>
                ...
                <td class="text-end">
                  <div class="action-stack">
                    <a class="btn btn-muted btn-sm w-100 w-md-auto" href="<?php echo h($self); ?>?edit=<?php echo (int)$r['EmployeeID']; ?>">
...
<script>
(function(){
  // JS side limits (mirror of COL_LIMITS)
  var LIMITS = {
    EmployeeCode: 50,
    FirstName: 100,
    LastName: 100,
  ...
  function validateOne(input, form){
    ...
    if(REQUIRED.indexOf(name) !== -1){
      if((!isSelect && val === '') || (isSelect && (val === '' || val === null))){
    ...
    switch(name){
      ...
      case 'Phone2':
        if(val){
          ...
          if(d.length > LIMITS[name]){ setInvalid(input, 'Max '+LIMITS[name]+' digits'); return false; }
          if(d && !/^\d+$/.test(d)){ setInvalid(input, 'Digits only'); return false; }
          if(val !== d){ input.value = d; }
        }
        break;
  ...
  function validateForm(form){
    ...
    ['Email_Office','Email_Personal'].forEach(function(n){
      ...
      if(el && el.value){ el.value = el.value.trim().toLowerCase(); }
    });

    $all('input, select, textarea', form).forEach(function(inp){
      if(inp.getAttribute('name')){
        if(!validateOne(inp, form)){ ok = false; }
      }
    });
    return ok;
  }
